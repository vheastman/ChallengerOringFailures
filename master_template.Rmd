...
## III. Modeling O-ring Failures with Temperature and Pressure
...
$$
\mbox{logit}(\hat{\pi}) = 13.2924 + 0.0104\mbox{Pressure} - 0.228671\mbox{Temp}
$$


```{r}
#Initial model of pressure as binary
...
$$
\mbox{logit}(\hat{\pi}) = 13.678 + 1.5882\mbox{Pressure} - 0.2266\mbox{Temp}
$$
...
The likelihood ratio test evaluates the importance of each explanatory variable . For the first explanatory variable, pressure, we test the hypothesis: $H_{0}: \beta_{1} = 0$ vs $H_{a}: \beta_{1} \neq 0$. The test statistic for the continuous pressure variable model is $-2log(\Lambda) = 1.5331$ and the p-value is 0.215648 so we fail to reject the null hypothesis that pressure has no effect on o-ring failure at $\alpha=0.05$. Similarly, we fail to reject the  null hypothesis that pressure has no effect on o-ring failure at $\alpha=0.05$ in the binary case with $-2log(\Lambda) = 1.7403$ and the p-value is 0.187097.

For the second explanatory variable, temperature, we test the hypothesis: $H_{0}: \beta_{2} = 0$ vs $H_{a}: \beta_{2} \neq 0$. The first model test statistic is $-2log(\Lambda) =  7.7542 $ and the p-value is 0.005359 so we reject the null hypothesis and conclude that there is evidence of an effect of temperature on o-ring failure. We reach to the same conclusion in the second model.
