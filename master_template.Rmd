...
```
Since 200 psi had some interesting distinguishing qualities in relation to the other variables such as proportion of failures as well as the 75◦F failure point,we create a binarized version of our pressure variable where we distinguish by wether or not the the pressure observed is 200 psi or not. We will analyze the impact of this at our model selection stage.
...
mod_binomial.fit1<-glm(formula=O.ring.total/Number~Pressure+Temp,data=df,family=binomial(link = logit),weights=Number)
...
mod_binomial.fit2<-glm(formula=O.ring.total/Number~P200+Temp,data=df,family=binomial(link = logit),weights=Number)
...
```{r}
mod_binomial.fit_temp<-glm(formula=O.ring.total/Number~Temp,data=df,family=binomial(link = logit),weights=Number)
...
```{r}
binary.fit<-glm(O.ring ~ Temp, family = binomial(link = "logit"),
data = df) 
summary(binary.fit)
```
...
# Binomial Model
curve(expr = predict(object = mod_binomial.fit_temp, newdata = data.frame(Temp = x), type = "response") * 6, col = "red", add = TRUE, xlim = c(30,
90))
# Binary Model
curve(expr = (1 - (1 - predict(object = binary.fit, newdata = data.frame(Temp = x), type = "response"))^(1/6)) * 6, col = "blue", add = TRUE,
xlim = c(30, 90))
...
par(mfrow = c(1, 2))
# pi vs. temperature
curve(expr = exp(mod_binomial.fit_temp$coefficients[1] + mod_binomial.fit_temp$coefficients[2] *
x)/(1 + exp(mod_binomial.fit_temp$coefficients[1] + mod_binomial.fit_temp$coefficients[2] * x)), xlim = c(31, 81), col = "red", xlab = "Temperature",
...
type = "response") * 6, col = "red", xlim = c(31, 81), xlab = "Temperature", ylab = "Expected number of failures", panel.first = grid())
```
...
```{r,out.height="4in", out.width="0.48\\textwidth"}
curve(expr = exp(mod_binomial.fit_temp$coefficients[1] + mod_binomial.fit_temp$coefficients[2] *
x)/(1 + exp(mod_binomial.fit_temp$coefficients[1] + mod_binomial.fit_temp$coefficients[2] *
x)), ylim = c(0, 1), xlim = c(31, 81), col = "red", main = expression("Confidence Interval", hat(pi)), xlab = "Temperature", ylab = expression(hat(pi)),
panel.first = grid())
...
curve(expr=ci.pi(newdata=data.frame(Temp=x),mod.fit.obj = mod_binomial.fit_temp, alpha = 0.05)$lower, col = "blue", lty="dotdash",add=TRUE,xlim=c(31,81))

curve(expr=ci.pi(newdata=data.frame(Temp=x),mod.fit.obj = mod_binomial.fit_temp, alpha = 0.05)$upper, col = "blue", lty="dotdash",add=TRUE,xlim=c(31,81))
```
...
# Confidence interval 
CI.pi=ci.pi(newdata = predict.data, mod.fit.obj = mod_binomial.fit_temp, alpha = alpha)

```

The estimated probability of o-ring failure at 31 F is `r round(pi.hat,4)` with and `r round(number_fail,4)` of the 6 O-rings failing. The corresponding 95% Wald confidence interval is `r round(CI.pi,4)`. 

#### 5 (e) Bootstrap Estimates

```{r}
# First, fit on observed
out <- glm(formula=O.ring.total/Number~Temp, family = binomial(link = logit), data=df, weights=Number) 
summary(out)
...
for (i in 1:nboot) { #for each bootstrap sample
    ...
    out.star <- glm(O.star/Number ~Temp, family = binomial(link = logit),data=samp_df,weights=Number) 
...
```
The 90% confidence interval was calculated using temperatures of 31$^\circ$F and 72$^\circ$F. At each tempera- ture, the confidence interval is determined by ordering the points and then choosing the values that correspond to the 5th percentile and 95th percentile.

At 31$^\circ$F, the 90% confidence interval of $$\hat{\pi}\ast$$ from the simluated model is `r ci.31`. At 72$^\circ$F, the 90% confidence interval of $$\hat{\pi}\ast$$ from the simluated model is `r ci.72`.

The bootstrapped 90% confidence interval for 31F is `r round(ci.31,4)` and for 72.27F is `r round(ci.72)`. 
...
```{r}
mod.fit.Ha<-glm(formula=O.ring.total/Number~Temp+I(Temp^2),data=df,family=binomial(link = logit),weights = Number)
anova(mod_binomial.fit_temp,mod.fit.Ha,test="Chisq")
```

The Quadratic term fails to produce significant effect in change in residual deviance and so we fail to reject that the coefficient is actually 0 for the quadratic term.
...
```{r}
mod.lm <- lm(O.ring ~ Temp, data = df)
summary(mod.lm)
```
The estimated linear regression model is

$$
\hat{\pi} = 2.90476  - 0.03738\mbox{Temp}
$$
